...
`!tabloid` or `!tb`: This is how you will tabloid others. You must send a photo with this command and you must mention at least one person in the photo. 

`!undo`: If your tabloid does not follow the rules (e.g., they saw you before you sent it) you can acknowledge your mistake and use the `!undo` command to remove this entry from the database. You must mention your victim. Leadership also has the ability to undo a tabloid, by using `!undo @tabloider @victim`. 
