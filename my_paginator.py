...
class Simple(discord.ui.View):
    ...
    async def start(self, ctx: discord.Interaction|commands.Context, pages: list[discord.Embed], sendAsDM: bool, user: discord.User):
        ...
        if(sendAsDM):
            self.message = await user.send(embed=self.pages[self.InitialPage], view=self)
        else:
